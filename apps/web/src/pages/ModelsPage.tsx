    ...
    <div className="stack">
      <section className="panel">
        <div className="panel-head">
          <h3>新增模型别名</h3>
          <span className="muted">模型别名会对外暴露给调用方。</span>
        </div>
      ...
      {models.map((model) => (
        <section key={model.id} className="panel">
          <div className="panel-head">
            <div>
              <h3>{model.alias}</h3>
              <p className="muted">绑定数：{model.bindings.length}</p>
            </div>
            <div className="action-row">
              ...
              <button
                ...
                onClick={() => {
                  if (!window.confirm(`确认删除模型 ${model.alias} 及其全部绑定吗？`)) {
                    return;
                  }
          ...
          <div className="form-grid">
            <label>
              ...
              <input
                value={model.alias}
                onChange={(event) => updateModelField(model.id, "alias", event.target.value)}
              />
            </label>
            <label>
              <span>展示名称</span>
              <input
                ...
                onChange={(event) =>
                  updateModelField(model.id, "displayName", event.target.value)
                }
              />
            </label>
          </div>
          <label className="inline">
            <input
              ...
              onChange={(event) => updateModelField(model.id, "enabled", event.target.checked)}
            />
            <span>模型启用</span>
          </label>
                  ...
                  <tr key={binding.id}>
                    <td>
                      <div className="action-row compact-row">
                        ...
                        <button
                          type="button"
                          className="chip"
                          onClick={() => moveBinding(model.id, index, -1)}
                        >
                          上移
                        </button>
                        <button
                    ...
                    <td>
                      <input
                        ...
                        onChange={(event) =>
                          updateBindingField(model.id, binding.id, "upstreamModel", event.target.value)
                        }
                      />
                    </td>
                    <td>
                    ...
                    <td>
                      <input
                        ...
                        onChange={(event) =>
                          updateBindingField(model.id, binding.id, "enabled", event.target.checked)
                        }
                      />
                    </td>
                    <td>
                      <div className="action-row">
                        <button
                          ...
                          onClick={() => {
                            void api.models
                              .updateBinding(model.id, binding.id, {
                        ...
                        <button
                          ...
                          onClick={() => {
                            void api.models
                              .removeBinding(model.id, binding.id)
                              .then(async () => {
                                await refreshModels();
          ...
          <div className="action-row">
            <button
              ...
              onClick={() => {
                void api.models
                  ...
                  .then(async () => {
                    await refreshModels();
                    onNotice(`模型 ${model.alias} 的运行顺序已应用`);
                  })
                  .catch(onError);
              }}
            >
            ...
            <button
              ...
              onClick={() => {
                void api.models
                  .saveDefaultOrder(model.id)
                  .then(async () => {
                    await refreshModels();
                    onNotice(`模型 ${model.alias} 的默认顺序已保存`);
                  })
                  .catch(onError);
              }}
            >
          ...
          <div className="form-grid">
            <label>
              <span>Provider</span>
              <select
                value={bindingDrafts[model.id]?.providerId ?? providers[0]?.id ?? ""}
                onChange={(event) =>
                  setBindingDrafts((current) => ({
                    ...current,
                    [model.id]: {
                      ...(current[model.id] ?? emptyBinding),
            ...
            <label>
              <span>上游模型名</span>
              <input
                value={bindingDrafts[model.id]?.upstreamModel ?? ""}
                onChange={(event) =>
                  setBindingDrafts((current) => ({
                    ...current,
                    [model.id]: {
                      ...(current[model.id] ?? emptyBinding),
                      providerId: current[model.id]?.providerId ?? providers[0]?.id ?? "",
            ...
            <label>
              ...
              <input
                type="number"
                step="0.0001"
                value={bindingDrafts[model.id]?.inputPrice ?? 0}
                onChange={(event) =>
                  setBindingDrafts((current) => ({
                    ...current,
                    [model.id]: {
            ...
            <label>
              ...
              <input
                type="number"
                step="0.0001"
                value={bindingDrafts[model.id]?.outputPrice ?? 0}
                onChange={(event) =>
                  setBindingDrafts((current) => ({
                    ...current,
                    [model.id]: {
